...
negbinom_quad_cv <- lapply(1:k, function(i){
  ...
  system.time({
    out <- jags(data = data, parameters.to.save = params, 
                model.file = "1_scripts/model-scripts/negbinom_quad.txt", n.chains = nc, n.iter = ni, 
...
save(negbinom_quad_cv, file = "2_pipeline/store/negbinom_quad_cv.Rdata")
