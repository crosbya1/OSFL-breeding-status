...
poisson_quad_cv <- lapply(1:k, function(i){
  ...
  system.time({
    out <- jags(data = data, parameters.to.save = params, 
                model.file = "1_scripts/model-scripts/poisson_quad.txt", n.chains = nc, n.iter = ni, 
...
save(poisson_quad_cv, file = "2_pipeline/store/poisson_quad_out.Rdata")
